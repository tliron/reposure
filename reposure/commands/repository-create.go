...
func init() {
	...
	repositoryCreateCommand.Flags().StringVarP(&provider, "provider", "", "", "registry provider (\"reposure\", \"minikube\", or \"openshift\")")
	repositoryCreateCommand.Flags().BoolVarP(&wait, "wait", "w", false, "wait for repository surrogate to come up")
}
...
var repositoryCreateCommand = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if host != "" {
			...
			switch provider {
			case "reposure":
				service = "reposure-registry"
				if tlsSecret == "" {
					tlsSecret = "reposure-registry"
				}
			...
			default:
				util.Fail("unsupported \"--provider\": must be \"reposure\", \"minikube\", or \"openshift\"")
			}
		}
		...
		var err error
		if service != "" {
			_, err = adminClient.CreateRepositoryIndirect(namespace, repositoryName, serviceNamespace, service, port, tlsSecret, tlsSecretDataKey, authSecret)
		} else {
			_, err = adminClient.CreateRepositoryDirect(namespace, repositoryName, host, tlsSecret, tlsSecretDataKey, authSecret)
		}
		util.FailOnError(err)

		if wait {
			_, err = adminClient.WaitForRepositorySurrogate(namespace, repositoryName)
			util.FailOnError(err)
		}
	},
}
